...
2) Using the built $connection object you will ask Twitter for temporary credentials. The `oauth_callback` value is
   required.
...
9) And finally we can make requests authenticated as the user. You can GET, POST, and DELETE API
   ...
   $status = $connection->post('statuses/update', array('status' => 'Text of status here', 'in_reply_to_status_id' =>
   123456));
