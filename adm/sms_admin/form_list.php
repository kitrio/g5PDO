...
?>
    ...
    <div class="local_sch01 local_sch sms_preset_sch">
        <form>
            ...
            <select name="fg_no" id="fg_no"
                    onchange="location.href='<?php echo $_SERVER['SCRIPT_NAME'] ?>?fg_no='+this.value;">
                <option value="" <?php echo $fg_no ? '' : 'selected' ?>> 전체</option>
                <option value="0" <?php echo $fg_no == '0' ? 'selected' : '' ?>> 미분류
                    (<?php echo number_format($no_count) ?>)
                </option>
                <?php for ($i = 0; $i < count($group); $i++) { ?>
                    <option value="<?php echo $group[$i]['fg_no'] ?>" <?php echo ($fg_no == $group[$i]['fg_no']) ? 'selected' : '' ?>> <?php echo $group[$i]['fg_name'] ?>
    ...
    <form name="emoticonlist" id="emoticonlist" method="post" action="./form_multi_update.php"
          onsubmit="return emoticonlist_submit(this);">
        ...
        <ul id="sms5_preset" class="sms5_box">
            ...
            for ($i = 0; $res = sql_fetch_array($qry); $i++) {
                ...
                <li class="li_<?php echo $li_i; ?> sms5_box">
                    ...
                    <div class="li_chk">
                        <label for="fo_no_<?php echo $i; ?>" class="sound_only"><?php echo $group_name ?>
                            의 <?php echo cut_str($res['fo_name'], 10) ?></label>
                        <input type="checkbox" name="fo_no[]" value="<?php echo $res['fo_no'] ?>"
                               id="fo_no_<?php echo $i; ?>">
                    </div>
                    <div class="li_preview">
                        <textarea readonly
                                  class="box_txt box_square"><?php echo html_purifier($res['fo_content']); ?></textarea>
                    </div>
                    <div class="li_info">
        ...
        <div class="btn_fixed_top">
            ...
            <input type="submit" name="act_button" value="선택삭제" onclick="document.pressed=this.value"
                   class="btn btn_02">
            <a href="./form_write.php?page=<?php echo $page ?>&amp;fg_no=<?php echo $fg_no ?>" class="btn btn_01">이모티콘
                추가</a>
        </div>
