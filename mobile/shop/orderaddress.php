                    ...
                    for ($i = 0; $row = sql_fetch_array($result); $i++) {
                        ...
                        <li>
                            <div class="addr_title chk_box">
                                <input type="hidden" name="ad_id[<?php echo $i; ?>]"
                                       value="<?php echo $row['ad_id']; ?>">
                                <input type="checkbox" name="chk[]" value="<?php echo $i; ?>" id="chk_<?php echo $i; ?>"
                                       class="ad_chk selec_chk">
                                <label for="chk_<?php echo $i; ?>"><span></span><strong
                                            class="sound_only">배송지선택</strong></label>
                                <label for="ad_subject<?php echo $i; ?>" class="sound_only">배송지명</label>
                                <input type="text" name="ad_subject[<?php echo $i; ?>]"
                                       value="<?php echo $row['ad_subject']; ?>" class="ad_subject" maxlength="20">
                            </div>
                            <div class="addr_info">
                                ...
                                <div class="addr_tel"><i class="fa fa-phone"
                                                         aria-hidden="true"></i> <?php echo $row['ad_tel']; ?> / <i
                                            class="fa fa-mobile" aria-hidden="true"></i> <?php echo $row['ad_hp']; ?>
                                </div>
                            </div>
                            <div class="addr_btn">
                                <input type="hidden" value="<?php echo $addr; ?>">
                                <button type="button" class="btn_sel sel_address">선택</button>
                                <a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?w=d&amp;ad_id=<?php echo $row['ad_id']; ?>"
                                   id="btn_del" class="del_address">삭제</a>
                                <input type="radio" name="ad_default" value="<?php echo $row['ad_id']; ?>"
                                       id="ad_default<?php echo $i; ?>" <?php if ($row['ad_default']) echo 'checked="checked"'; ?>>
